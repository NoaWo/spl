...
class Dao(object):
    ...
    def find_all_order(self, key_order):
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY {} ASC'.format(self._table_name, key_order))
        return orm(c, self._dto_type)

    def find_all_order_list(self, key_order):
        c = self._conn.cursor()
        c.execute('SELECT * FROM {} ORDER BY {} ASC'.format(self._table_name, key_order))
        return c.fetchall()  
    ...
    def update(self, set_values, cond):
        ...
        stmt = 'UPDATE {} SET {} WHERE {}'.format(self._table_name, 
                                                    ', '.join([set1 + '=?' for set1 in set_column_names]),
